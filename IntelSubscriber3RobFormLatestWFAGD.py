...
class IntelSubscriber:
    def __init__(self):      
       self.node = rclpy.create_node('intel_subscriber_node') 
       self.bridge = CvBridge()
       self.rgb_sub = self.node.create_subscription(Image, '/realsense/camera/rgb/image_raw', self.IntelSubscriberRGB, 10)
    ...
    def IntelSubscriberRGB(self,msg):
       ...
       try:
          self.node.get_logger().info('Receiving RGB data')
          rgb_image = self.bridge.imgmsg_to_cv2(msg, "bgr8")
          cv2.waitKey(1)
       except Exception as e:
          self.node.get_logger().error(f'Error converting RGB image: {e}')
          
       camera_msg = rclpy.wait_for_message('/realsense/camera/rgb/camera_info', CameraInfo)
       depth_msg = rclpy.wait_for_message('/realsense/camera/depth/image_raw', Image)
...
def main(args = None):
      try:
       ...
       node = rclpy.create_node('realsense_subscriber_rgb')
       ...
       velocity_pub1 = node.create_publisher(Twist, 'tb3_0/cmd_vel', 10)
       pose_subscriber1 = node.create_subscription(Odometry, 'tb3_0/odom', server1.odom_callback, 10)
       lidar_subscriber1 = node.create_subscription(LaserScan, 'tb3_0/scan', server1.lidar_callback, 10)
       velocity_pub2 = node.create_publisher(Twist, 'tb3_1/cmd_vel', 10)
       pose_subscriber2 = node.create_subscription(Odometry, 'tb3_1/odom', server2.odom_callback, 10)
       lidar_subscriber2 = node.create_subscription(LaserScan, 'tb3_1/scan', server2.lidar_callback, 10)
       velocity_pub3 = node.create_publisher(Twist, 'tb3_2/cmd_vel', 10)
       pose_subscriber3 = node.create_subscription(Odometry, 'tb3_2/odom', server3.odom_callback, 10)
       lidar_subscriber3 = node.create_subscription(LaserScan, 'tb3_2/scan', server3.lidar_callback, 10)
