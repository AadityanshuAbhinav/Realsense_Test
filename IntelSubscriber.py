...
global s,ite,move1,move2,move3,poseid,poseidf,poseidin,flaglist,initial_time,v,i5list,i4list,i3list,n,poseidPrev,xErrT,yErrT,NT
...
z_anglist = None
poseid = np.zeros([NT,3])
poseidf = np.zeros([NT,3])
poseidin = np.zeros([NT,3])
posehist = np.zeros([n,3,NT])
poseidPrev = np.empty([NT,3])
...
class IntelSubscriber(Node):
   def __init__(self):     
      ...
      self.velocity_pub1 = self.create_publisher(Twist, 'rbot0/cmd_vel', 10)
      self.velocity_pub2 = self.create_publisher(Twist, 'rbot1/cmd_vel', 10)
      self.velocity_pub3 = self.create_publisher(Twist, 'rbot2/cmd_vel', 10)
   ...
   def IntelSubscriberRGB(self, msg):
      ...
      if self.depth_image is not None:
         ...
         Coordinates_list = []
         z_anglist = []
         poseid = np.empty([NT,3])
         poseidf = np.empty([NT,3])
         ...
         self.RelController(poseid[4,:],RelPosition,Form[0][:],goal,FormErrMat1,DistErrMat1,move1,0,c_mat1,ctime_mat1,4,i5list,PI_error1,xdot_mat1,xerr_mat1)
         ...
         self.velocity_pub2.publish(move2)
         self.velocity_pub3.publish(move3)
   ...
   def RelController(self,poseid,RelPose,Form,goallist,Formerror,Disterror,move,lead,c_mat,ctime_mat,idd,ilist,PI_error,x_dotmat,x_err):
      ...
      Rely = 0
      for i in ilist:
         Relx = Relx + RelPose[idd*NT+i,0]
         Rely = Rely + RelPose[idd*NT+i,1]
      ...
      print("xformerror",x_form_error,"yform_error",y_form_error)
      ...
      if lead==1:
         if distance_to_pose < 0.05 and distance_to_goal < 0.05:
            move.linear.x = 0.0
            move.angular.z = 0.0#K_ang*((rob_theta)*np.pi/180)
         else:
   ...
   def RelControllerAGD(self,poseid,poseidPrev,RelPose,RelPosePrev,Form,goallist,Formerror,Disterror,move,lead,c_mat,ctime_mat,idd,ilist,PI_error,x_dotmat,x_err):
      ...
      Rely = 0
      for i in ilist:
         Relx = Relx + Beta1*(RelPose[idd*NT+i,0]-RelPosePrev[idd*NT+i,0]) + RelPose[idd*NT+i,0]	
         Rely = Rely + Beta1*(RelPose[idd*NT+i,1]-RelPosePrev[idd*NT+i,1]) + RelPose[idd*NT+i,1]
         
      Relx1 = 0
      Rely1 = 0
      for i in ilist:
         Relx1 = Relx1 + RelPose[idd*NT+i,0]	
         Rely1 = Rely1 + RelPose[idd*NT+i,1]
   ...
   def run(self):
      ...
      velocity_pub2 = self.create_publisher(Twist, 'tb1/cmd_vel', 10)
      pose_subscriber2 = self.create_subscription(Odometry, 'tb1/odom', server2.odom_callback, 10)
      lidar_subscriber2 = self.create_subscription(LaserScan, 'tb1/scan', server2.lidar_callback, 10)

      velocity_pub3 = self.create_publisher(Twist, 'tb2/cmd_vel', 10)
      pose_subscriber3 = self.create_subscription(Odometry, 'tb2/odom', server3.odom_callback, 10)
      lidar_subscriber3 = self.create_subscription(LaserScan, 'tb2/scan', server3.lidar_callback, 10)
